...
// ── アフィリエイト商品データ ────────────────────────────────
// ★ Amazon アソシエイトのタグIDを設定してください
// https://affiliate.amazon.co.jp/ で登録後、タグIDを取得できます
const AFFILIATE_TAG = 'YOUR-TAG-22'; // ← ここを自分のタグIDに変更

const _amz = kw => `https://www.amazon.co.jp/s?k=${encodeURIComponent(kw)}&tag=${AFFILIATE_TAG}`;

const AFFILIATE_PRODUCTS = {
  // ロードマップ各フェーズに表示する準備リスト
  phase: {
    early: [
      { name: '葉酸サプリ',     emoji: '💊', note: '妊娠初期〜授乳期まで摂取推奨',         url: _amz('葉酸サプリ 妊婦') },
      { name: 'マタニティ本',   emoji: '📚', note: '産婦人科医監修の定番書籍',             url: _amz('妊娠 出産 本 おすすめ') },
      { name: 'マタニティ下着', emoji: '👙', note: 'お腹が大きくなる前に準備を',           url: _amz('マタニティ ブラ ショーツ セット') },
    ],
    mid: [
      { name: 'マタニティウェア', emoji: '👗', note: 'お腹が目立ち始める中期から',         url: _amz('マタニティ ワンピース') },
      { name: '腹帯・骨盤ベルト', emoji: '🩹', note: '腰痛対策・お腹サポートに',           url: _amz('マタニティ 腹帯 骨盤ベルト') },
      { name: '抱き枕',           emoji: '🛏', note: '横向き寝がラクになる',               url: _amz('マタニティ 抱き枕') },
    ],
    late: [
      { name: '入院バッグ',         emoji: '🧳', note: '出産入院に必要なものをセットで',   url: _amz('マタニティ 入院バッグ セット') },
      { name: 'マタニティパジャマ', emoji: '🌙', note: '入院中・産後の授乳にも使える',     url: _amz('マタニティ パジャマ 授乳') },
      { name: '骨盤ベルト（産後）', emoji: '🩹', note: '出産後の骨盤ケアにも',             url: _amz('骨盤ベルト 産後 矯正') },
    ],
    birth: [
      { name: 'チャイルドシート', emoji: '🚗', note: '退院時に必須。新生児から使える回転式', url: _amz('チャイルドシート 新生児 回転式') },
      { name: 'ベビーカー',       emoji: '🍼', note: '生後1ヶ月から使える両対面タイプ',     url: _amz('ベビーカー 両対面 新生児') },
    ],
    postnatal: [
      { name: '授乳クッション',       emoji: '🤱', note: '授乳姿勢をラクにする',           url: _amz('授乳クッション') },
      { name: 'ベビーモニター',       emoji: '📷', note: '就寝中の見守りに安心',           url: _amz('ベビーモニター カメラ') },
      { name: 'おむつ（まとめ買い）', emoji: '🧷', note: '定期便で割引になる',             url: _amz('新生児 おむつ まとめ買い') },
    ],
    childcare: [
      { name: '離乳食グッズ',     emoji: '🥄', note: '5〜6ヶ月頃から準備を',             url: _amz('離乳食 グッズ セット 初めて') },
      { name: 'ベビーチェア',     emoji: '🪑', note: '離乳食開始時に必要',               url: _amz('ベビーチェア ハイチェア') },
      { name: 'ベビー服セット',   emoji: '👶', note: 'すぐサイズアウトするので最小限に', url: _amz('ベビー服 60 70 セット') },
    ],
  },
  // カレンダー週数カード（展開時）に表示する商品。該当週のみ設定
  week: {
    8:  [{ name: 'つわり対策グッズ', emoji: '🌿', note: 'ショウガキャンディ・炭酸水・ツボ押しリストバンドなど', url: _amz('つわり 対策 グッズ') }],
    20: [{ name: '妊婦帯・腹帯',     emoji: '🩹', note: 'お腹が目立つ時期のサポートに',                       url: _amz('妊婦帯 腹帯 トコちゃん') }],
    28: [{ name: 'マタニティ抱き枕', emoji: '🛏', note: '後期の寝苦しさを軽減。授乳クッションにも使える',     url: _amz('マタニティ 抱き枕 授乳クッション') }],
    32: [{ name: '入院準備セット',   emoji: '🧳', note: '入院バッグ・産褥ショーツ・授乳ブラなど',             url: _amz('出産 入院 準備 セット') }],
    37: [{ name: 'チャイルドシート', emoji: '🚗', note: '退院時に必要。早めの準備を',                         url: _amz('チャイルドシート 新生児 回転式') }],
  },
};
