...
// ── アフィリエイト商品データ ────────────────────────────────
// ★ Amazon アソシエイトのタグIDを設定してください（https://affiliate.amazon.co.jp/）
const AFFILIATE_TAG  = 'YOUR-TAG-22';      // ← Amazonアソシエイトのタグに変更
// ★ 楽天アフィリエイトIDを設定してください（https://affiliate.rakuten.co.jp/）
const RAKUTEN_AID    = 'YOUR-RAKUTEN-AID'; // ← 楽天アフィリエイトIDに変更

const _amz  = kw => `https://www.amazon.co.jp/s?k=${encodeURIComponent(kw)}&tag=${AFFILIATE_TAG}`;
const _rktn = kw => `https://search.rakuten.co.jp/search/mall/${encodeURIComponent(kw)}/?scid=af_pc_etc&sc2id=af_101_${RAKUTEN_AID}`;
const _prod = (name, emoji, note, kw) => ({ name, emoji, note, amzUrl: _amz(kw), rktnUrl: _rktn(kw) });

const AFFILIATE_PRODUCTS = {
  // ロードマップ各フェーズに表示する準備リスト
  phase: {
    early: [
      _prod('葉酸サプリ',     '💊', '妊娠初期〜授乳期まで摂取推奨',         '葉酸サプリ 妊婦'),
      _prod('マタニティ本',   '📚', '産婦人科医監修の定番書籍',             '妊娠 出産 本 おすすめ'),
      _prod('マタニティ下着', '👙', 'お腹が大きくなる前に準備を',           'マタニティ ブラ ショーツ セット'),
    ],
    mid: [
      _prod('マタニティウェア', '👗', 'お腹が目立ち始める中期から',         'マタニティ ワンピース'),
      _prod('腹帯・骨盤ベルト', '🩹', '腰痛対策・お腹サポートに',           'マタニティ 腹帯 骨盤ベルト'),
      _prod('抱き枕',           '🛏', '横向き寝がラクになる',               'マタニティ 抱き枕'),
    ],
    late: [
      _prod('入院バッグ',         '🧳', '出産入院に必要なものをセットで',   'マタニティ 入院バッグ セット'),
      _prod('マタニティパジャマ', '🌙', '入院中・産後の授乳にも使える',     'マタニティ パジャマ 授乳'),
      _prod('骨盤ベルト（産後）', '🩹', '出産後の骨盤ケアにも',             '骨盤ベルト 産後 矯正'),
    ],
    birth: [
      _prod('チャイルドシート', '🚗', '退院時に必須。新生児から使える回転式', 'チャイルドシート 新生児 回転式'),
      _prod('ベビーカー',       '🍼', '生後1ヶ月から使える両対面タイプ',     'ベビーカー 両対面 新生児'),
    ],
    postnatal: [
      _prod('授乳クッション',       '🤱', '授乳姿勢をラクにする',           '授乳クッション'),
      _prod('ベビーモニター',       '📷', '就寝中の見守りに安心',           'ベビーモニター カメラ'),
      _prod('おむつ（まとめ買い）', '🧷', '定期便で割引になる',             '新生児 おむつ まとめ買い'),
    ],
    childcare: [
      _prod('離乳食グッズ',   '🥄', '5〜6ヶ月頃から準備を',             '離乳食 グッズ セット 初めて'),
      _prod('ベビーチェア',   '🪑', '離乳食開始時に必要',               'ベビーチェア ハイチェア'),
      _prod('ベビー服セット', '👶', 'すぐサイズアウトするので最小限に', 'ベビー服 60 70 セット'),
    ],
  },
  // カレンダー週数カード（展開時）に表示する商品。該当週のみ設定
  week: {
    8:  [_prod('つわり対策グッズ', '🌿', 'ショウガキャンディ・炭酸水・ツボ押しリストバンドなど', 'つわり 対策 グッズ')],
    20: [_prod('妊婦帯・腹帯',     '🩹', 'お腹が目立つ時期のサポートに',                       '妊婦帯 腹帯 トコちゃん')],
    28: [_prod('マタニティ抱き枕', '🛏', '後期の寝苦しさを軽減。授乳クッションにも使える',     'マタニティ 抱き枕 授乳クッション')],
    32: [_prod('入院準備セット',   '🧳', '入院バッグ・産褥ショーツ・授乳ブラなど',             '出産 入院 準備 セット')],
    37: [_prod('チャイルドシート', '🚗', '退院時に必要。早めの準備を',                         'チャイルドシート 新生児 回転式')],
  },
};
